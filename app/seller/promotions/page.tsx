...
export default function SellerPromotionsPage() {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await authenticatedFetch<any>(API_ENDPOINTS.SELLERS.PROMOTIONS, {
  ...
  const toggle = async (id: string, currentStatus: boolean) => {
    ...
    try {
      await authenticatedFetch(`${API_ENDPOINTS.SELLERS.PROMOTIONS}/${id}/toggle`, {
        method: 'PUT',
        accessToken: session.accessToken,
      });
    } catch (err) {
  ...
  const del = async (id: string) => {
    if (!session?.accessToken) return;
    try {
       await authenticatedFetch(`${API_ENDPOINTS.SELLERS.PROMOTIONS}/${id}`, {
        method: 'DELETE',
        accessToken: session.accessToken,
      });
