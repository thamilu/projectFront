...
import { authenticatedFetch, FetchError } from '@/lib/utils/fetch-utils';
...
/** Order status label → step index mapping */
...
/** Calculates discount percentage, returns null when no valid discount */
...
/** Empty state used when API returns 403 (new / unsynced user) */
...
export default function CustomerDashboardPage() {
  ...
  const fetchData = useCallback(async (token: string) => {
    ...
    setError(null);
    try {
      const resp = await authenticatedFetch<any>(API_ENDPOINTS.DASHBOARD.CUSTOMER, { accessToken: token });
      const dashboardData = (resp && (resp.data ?? resp)) as CustomerDashboardData;
      ...
      if (isForbidden) {
        ...
        handleError(err, '[Dashboard] Fetch failed');
        setError(err instanceof Error ? err.message : getUserFriendlyMessage(err));
      }
    } finally {
      setLoading(false);
    }
