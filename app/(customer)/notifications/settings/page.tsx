...
const PREF_GROUPS: PrefGroup[] = [
  {
    title: 'Order Updates', icon: Package,
    prefs: [
  ...
  {
    title: 'Account', icon: Star,
    prefs: [
      { key: 'review_reminder', label: 'Review Reminders', desc: 'Remind to review delivered orders' },
...
export default function NotificationSettingsPage() {
  const [prefs, setPrefs] = useState<Prefs>(
    Object.fromEntries(
      PREF_GROUPS.flatMap(g => g.prefs.map(p => [p.key, { email: true, sms: false, push: true }]))
    )
  );
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-10">
      ...
      {/* Channel Legend */}
      <div className="mb-6 flex flex-wrap gap-4 rounded-xl bg-muted/50 p-4 text-sm">
        <div className="flex items-center gap-1.5"><Mail className="h-4 w-4 text-blue-600" /> Email</div>
        <div className="flex items-center gap-1.5"><Smartphone className="h-4 w-4 text-green-600" /> SMS</div>
        <div className="flex items-center gap-1.5"><Bell className="h-4 w-4 text-purple-600" /> Push</div>
      </div>
                ...
                <div key={pref.key} className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                      <Switch id={`${pref.key}-email`} checked={!!prefs[pref.key]?.email} onChange={() => toggle(pref.key, 'email')} />
                      <Label htmlFor={`${pref.key}-email`} className="sr-only">Email</Label>
                      <Mail className="h-3.5 w-3.5 text-blue-500" />
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Switch id={`${pref.key}-sms`} checked={!!prefs[pref.key]?.sms} onChange={() => toggle(pref.key, 'sms')} />
                      <Label htmlFor={`${pref.key}-sms`} className="sr-only">SMS</Label>
                      <Smartphone className="h-3.5 w-3.5 text-green-500" />
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Switch id={`${pref.key}-push`} checked={!!prefs[pref.key]?.push} onChange={() => toggle(pref.key, 'push')} />
                      <Label htmlFor={`${pref.key}-push`} className="sr-only">Push</Label>
