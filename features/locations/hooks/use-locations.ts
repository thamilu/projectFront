...
export function useLocations(
  ...
  const isValidPincodeFormat = Boolean(isIndia && pincodeValue?.length === 6 && /^\d+$/.test(pincodeValue));

  // 1. Pincode Lookup (Primary)
  const { 
    data: pincodeData, 
    isLoading: isLoadingPincodeData 
  } = usePincodeDetailQuery(pincodeValue, isValidPincodeFormat);

  // 2. Pincode Search (Autocomplete)
  const { 
    data: apiSearchedPincodes, 
    isLoading: isLoadingPincodeSearch 
  } = usePincodeSearchQuery(pincodeValue, !!pincodeValue && pincodeValue.length >= 1 && pincodeValue.length <= 6);
  ...
  // 4. States
  const { data: apiStates } = useStatesQuery(isIndia);
  ...
  } = useDistrictsQuery(selectedStateId);
  ...
  const { 
    data: apiTaluks, 
    isLoading: isLoadingTaluks 
  } = useTaluksQuery(selectedDistrictId);
  ...
  } = usePincodesQuery(selectedTalukId ? 'taluk' : 'district', selectedTalukId || selectedDistrictId);
  ...
  return {
    ...
    isLoadingCountries,
    isLoadingStates: !apiStates && isIndia,
    isLoadingDistricts,
    isLoadingTaluks,
    isLoadingPincodes,
