/**
 * scripts/verify-keycloak-live.ts — run via `npm run check:keycloak-live`.
 *
 * Distinct from `npm run check:keycloak` (verify-keycloak-setup.ts), which
 * only checks that required files/env vars are present. This script makes a
 * real network call to Keycloak's well-known OIDC endpoint (and, if a client
 * secret is available, a live client_credentials token request) — use it
 * when you need to confirm the configured Keycloak instance is actually
 * reachable and responding correctly, not just that the local setup looks right.
 */
import fs from 'fs';
