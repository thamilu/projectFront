...
export function extractNames(content: string): string[] {
  ...
  if (exportBlockMatch) {
    ...
    for (let p of parts) {
      ...
      if (!p) continue;
      if (
        p.includes('createLucideIcon') ||
        p.includes('index as icons') ||
        p.startsWith('type ') ||
        p.includes('LucideProps')
      )
        continue;
...
export function generateOutput(filtered: string[]): string {
  ...
  outLines.push('// Generated by scripts/generate-lucide-types.ts (run `npm run check:icons` to verify, no args to regenerate) — do not edit by hand');
...
async function main() {
  ...
  const outFile = path.join(__dirname, '..', 'shared', 'types', 'lucide-icons.d.ts');
  ...
  if (checkMode) {
    ...
    if (existing === out) {
      ...
      console.error(
        'ERROR: --check: generated output differs; run `node scripts/generate-lucide-types.ts` to regenerate.'
      );
      process.exit(3);
    }
  }
...
if (
  ...
  (require.main.filename.endsWith('generate-lucide-types.ts') ||
    require.main.filename.endsWith('generate-lucide-types.js'))
) {
  main().catch((e) => {
